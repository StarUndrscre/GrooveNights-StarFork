...
function ComboContinuesBetweenSongs()
	local t = OptionRowBase('ComboContinuesBetweenSongs')
	t.LayoutType = 'ShowAllInRow'
	t.OneChoiceForAllPlayers = true
	t.Choices = { "Off", "On" }
	t.LoadSelections = function(self, list) if PREFSMAN:GetPreference('ComboContinuesBetweenSongs') == 0 then list[1] = true; else list[2] = true end end
	t.SaveSelections = function(self, list)
		if list[1] then PREFSMAN:SetPreference('ComboContinuesBetweenSongs',false) end
		if list[2] then PREFSMAN:SetPreference('ComboContinuesBetweenSongs',true) end
	end
	return t
end

function ShowStageNumberOnGameplayScreen()
	local t = OptionRowBase('ShowStageNumberOnGameplayScreen')
	t.LayoutType = 'ShowAllInRow'
	t.OneChoiceForAllPlayers = true
	t.Choices = { "Off", "On" }
	t.LoadSelections = function(self, list) if PREFSMAN:GetPreference('ShowStageNumberOnGameplayScreen') == 0 then list[1] = true; else list[2] = true end end
	t.SaveSelections = function(self, list)
		if list[1] then PREFSMAN:SetPreference('ShowStageNumberOnGameplayScreen',false) end
		if list[2] then PREFSMAN:SetPreference('ShowStageNumberOnGameplayScreen',true) end
	end
	return t
end

function ComboShowLeadingZeroes()
	local t = OptionRowBase('ComboShowLeadingZeroes')
	t.LayoutType = 'ShowAllInRow'
	t.OneChoiceForAllPlayers = true
	t.Choices = { "Off", "On" }
	t.LoadSelections = function(self, list) if PREFSMAN:GetPreference('ComboShowLeadingZeroes') == false then list[1] = true; else list[2] = true end end
	t.SaveSelections = function(self, list)
		if list[1] then PREFSMAN:SetPreference('ComboShowLeadingZeroes',false) end
		if list[2] then PREFSMAN:SetPreference('ComboShowLeadingZeroes',true) end
	end
	return t
end
...
function EnableGlobalPlayerStats()
	local t = OptionRowBase('EnableGlobalPlayerStats')
	t.LayoutType = 'ShowAllInRow'
	t.OneChoiceForAllPlayers = true
	t.Choices = { "Card Only", "Global Profile/Card", "Global Profile For P1", }
	t.LoadSelections = function(self, list) local playerstattype = PROFILEMAN:GetMachineProfile():GetSaved().gnPlayerStatsType; if playerstattype == nil then list[1] = true; return end; list[playerstattype + 1] = true end
	t.SaveSelections = function(self, list)
		if list[1] then PROFILEMAN:GetMachineProfile():GetSaved().gnPlayerStatsType = 0 end
		if list[2] then PROFILEMAN:GetMachineProfile():GetSaved().gnPlayerStatsType = 1 end
		if list[3] then PROFILEMAN:GetMachineProfile():GetSaved().gnPlayerStatsType = 2 end
	end
	return t
end

function EnableQuickPlayMenuOnTitle()
	local t = OptionRowBase('EnableQuickPlayMenuOnTitle')
	t.LayoutType = 'ShowAllInRow'
	t.OneChoiceForAllPlayers = true
	t.Choices = { "Off", "On" }
...
function SlumpageCheck( pn )
	local steps = GAMESTATE:GetCurrentSteps( pn ) 
	local allsteps = GAMESTATE:GetCurrentSong():GetAllSteps()
	local totalsteps = table.getn(allsteps)
	local description = string.lower(GAMESTATE:GetCurrentSteps( pn ):GetDescription())

	if string.find(description,'slu') ~= nil then
		return true
	elseif steps:GetDifficulty() == 5 and totalsteps == 2 then
		local reg = 0
		local edit = 0
		if steps:GetDifficulty() == allsteps[2]:GetDifficulty() then 
			edit = 2
			reg = 1
		else
			edit = 1
		...
		if allsteps[edit]:GetMeter() > allsteps[reg]:GetMeter() then 
			return true
		end
		return false
	end
